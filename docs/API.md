...
Assigns the next sequential XBAN to `target`.
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| number | uint64 | Assigned XBAN account number. |
...
Emitted whenever an XBAN is assigned.
...
| target | address | Address associated with the XBAN. |
