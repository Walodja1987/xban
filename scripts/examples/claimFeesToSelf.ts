/**
 * Script to claim accumulated fees from XBAN and send them to yourself (claimFeesToSelf)
 *
 * USAGE:
 * Run the script with:
...
async function main() {
  ...
  // Check pending fees
  const getPendingFees = xban.getFunction("getPendingFees(address)");
  ...
  const claimTx = await xban.connect(signer).claimFeesToSelf();
