/**
 * Script to initiate ownership transfer of the XBAN contract
 *
 * USAGE:
 * Run the script with:
...
async function main() {
  ...
  // Check current owner
  const currentOwnerAddress = await xban.owner();
  ...
  const existingPendingOwner = await xban.pendingOwner();
  ...
  const transferTx = await xban.connect(currentOwner).transferOwnership(newOwner);
  ...
  const pendingOwner = await xban.pendingOwner();
  const ownerAfter = await xban.owner();
